...
当前地点搜索/附近查询和 `POST /api/v1/marine-analyses` 的 Web 边界已实现参数、坐标成对性、经纬度范围、起始时间和 `24/72/168` 小时校验，返回 `400` 的 `VALIDATION_FAILED` ProblemDetails；分析请求的未知 `locationId` 返回 `404 LOCATION_NOT_FOUND`；Weather/Marine Provider 无法提供结果且没有可用降级值时返回 `503` 的 `PROVIDER_UNAVAILABLE` ProblemDetails。响应都携带 `Trace-Id` 响应头和 `traceId` 扩展字段。缓存后端故障仍按缓存边界设计绕过，不直接转为 API 错误。
...
| 1.1 | 2026-07-16 | 补充 metrics-only 分析查询的验证失败、Provider 不可用和 Trace-Id 映射 |
