...
当前 `POST /api/v1/marine-analyses` 的 Web 边界已实现两类映射：请求体、坐标成对性、经纬度范围、起始时间和 `24/72/168` 小时校验返回 `400` 的 `VALIDATION_FAILED` ProblemDetails；Weather/Marine Provider 无法提供结果且没有可用降级值时返回 `503` 的 `PROVIDER_UNAVAILABLE` ProblemDetails。两类响应都携带 `Trace-Id` 响应头和 `traceId` 扩展字段。缓存后端故障仍按缓存边界设计绕过，不直接转为 API 错误。
