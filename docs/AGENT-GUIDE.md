...
| 当前任务 ID | `MI-0040` |
| 当前状态 | `DONE` |
| 当前目标 | 首页查询交互优化与 AI 解读时区统一（2026-08-19 需求 1-5） |
| 最后完成动作 | 需求1：「查找地点」命中预置地点后地图跟随显示标记，未命中提示走地图选点，**地图默认收起**（`IsMapPickerOpen` 默认 false，点击「地图选点」才展开，`SearchLocationsAsync`/`SelectLocationAsync` 命中即打开地图并 `UpdateMapSelectionAsync`）；需求2：预置地点后台新增「设为首页默认地点」选项，`locations.is_home_default` SQLite/PostgreSQL 双迁移（默认 false，设新默认自动清旧默认），`DashboardQuerySession.SelectHomeDefaultAsync` 无查询参数时预选首页默认地点；需求3：`dashboard-map.js` 缩放控件附近新增定位十字准星控件，首次加载不请求权限、用户主动点击才 `getCurrentPosition`（成功后 `map.setView` + `SelectMapPointFromJs` 标点跟随显示，失败经 `HandleLocateUnavailableFromJs`→`SetMapError` 给可恢复提示）；需求4：选点后经天地图服务端逆地理编码反查最近地名填充「地点名称（可选）」（新增 `IReverseGeocoder`/`TiandituReverseGeocoder`/`ReverseGeocodeService` + `GET /locations/reverse-geocode`，Best-effort 失败留空），并附提示该名称仅为地点备注；需求5：AI 解读与逐小时趋势/推荐时间窗/逐小时指标时区统一为用户显示时区（`MarineAnalysisRequest.TimeZone` 可选，`ExplanationFactsBuilder.ResolveTimeZoneId` 校验 IANA 并回退地点时区，SystemPrompt 规则 5 + `PromptVersion=explanation-prompt.v2` + 解释缓存键纳入时区） |
| 下一步动作 | 标准提交（`feat(dashboard): ...`）后按部署手册生产发布：tar+ssh 同步 `src/`+`deploy/postgresql-migrations.sql`+`deploy/docker-cleanup.sh`、pg_dump 备份、`up -d --build`（叠加 production/ai/tianditu overlay，`AddHomeDefaultLocation` 迁移自动应用）、冒烟验证健康检查/预置地点/首页/天地图、`bash deploy/docker-cleanup.sh`；git 推送 `origin` 若网络不可达则待用户经代理/GitHub Desktop 补齐 |
| 涉及文件 | `Dashboard.razor`、`DashboardQuerySession.cs`、`dashboard-map.js`、`app.css`、`Dashboard.razor.css`、`AdminLocations.razor`、`UserLocations.razor`、`AdminLocationService`/`IAdminLocationRepository`/`AdminLocationRepository`/`AdminModels`（`IsHomeDefault`）、`IReverseGeocoder.cs`、`ReverseGeocodeService.cs`、`TiandituOptions.cs`、`TiandituReverseGeocoder.cs`、`TiandituServiceCollectionExtensions.cs`、`LocationEndpointExtensions.cs`、`LocationContracts.cs`、`MarineAnalysisContracts.cs`、`MarineAnalysisEndpointExtensions.cs`、`ExplanationService.cs`、`ExplanationFactsBuilder.cs`、`ExplanationCacheIdentity.cs`、`ExplanationDefaults.cs`、`OpenAiCompatibleExplanationProvider.cs`、`Program.cs`、双迁移 `AddHomeDefaultLocation`、`deploy/postgresql-migrations.sql`、测试 ×6、`docs/05/06/07/10/15/16/21/22/AGENT-GUIDE` |
| 验证结果 | Release 构建 0 警告、0 错误；全量测试 229/229 通过（Domain 51、Application 69、Infrastructure 50、Web 59，新增/更新 11 例含 `TiandituReverseGeocoderTests`、时区统一与首页默认地点用例）；`dotnet format --verify-no-changes`、`git diff --check` 通过，改动文本文件 BOM/CRLF 已统一 |
| 阻塞/待确认 | 无 |
| 最后更新 | 2026-08-19 |
...
| --- | --- | --- | --- | --- |
| 2026-08-19 | `MI-0040` | 首页查询交互优化 + 天地图逆地理编码 + AI 时区统一（需求 1-5）：`DashboardQuerySession` 新增 `SelectHomeDefaultAsync`（无查询参数预选首页默认地点，缺省地图收起）、`SetMapError`（可恢复地图提示）、`ToggleMapPicker`；`Dashboard.razor` 「查找地点」/候选点击改走 `SearchLocationsAsync`/`SelectLocationAsync`（命中即打开地图并 `UpdateMapSelectionAsync` 标记跟随显示，未命中提示走地图选点），地图默认收起（`IsMapPickerOpen` 默认 false）；`dashboard-map.js` 新增 `addLocateControl` 定位十字准星（缩放控件附近，首次加载不请求权限，用户主动点击才 `getCurrentPosition`，成功 `map.setView` + `SelectMapPointFromJs` 标点跟随显示，失败经 `HandleLocateUnavailableFromJs`→`SetMapError` 中文可恢复提示）；新增 `IReverseGeocoder`/`TiandituReverseGeocoder`（`Map:Tianditu:Key` 复用浏览器瓦片 Key，key-per-file Secret）/`ReverseGeocodeService` + `GET /api/v1/locations/reverse-geocode`，`SelectMapPointFromJs` 变 async 反查最近地名填充「地点名称（可选）」（Best-effort 失败留空，附提示仅作地点备注）；预置地点后台新增「设为首页默认地点」（`locations.is_home_default` SQLite/PostgreSQL 双迁移 `AddHomeDefaultLocation`，默认 false、设新默认自动清旧默认）；AI 时区统一：`MarineAnalysisRequest.TimeZone` 可选（IANA），`ExplanationFactsBuilder.ResolveTimeZoneId` 校验并回退地点时区，SystemPrompt 规则 5 强制 AI/降级模板按该时区换算当地时间输出，`PromptVersion=explanation-prompt.v2`、解释缓存键纳入时区，逐小时趋势/推荐时间窗/逐小时指标显示时区均与用户一致 | Release 构建 0 警告、0 错误；全量测试 229/229 通过（Domain 51、Application 69、Infrastructure 50、Web 59，新增 `TiandituReverseGeocoderTests` 及首页默认地点、时区统一用例，更新 `DashboardQuerySessionTests`/`AuthenticationTests` 适配地图默认收起）；`dotnet format --verify-no-changes`、`git diff --check` 通过，改动文本文件 BOM/CRLF 已统一 | 已标准提交 `afaecdf`；git 推送 `origin` 因 `github.com:443` 不可达待用户补齐；2026-08-19 生产部署上线并冒烟验证通过（源码 tar+ssh 同步、pg_dump 备份 `marine-insight-20260819-041231.dump`、`AddHomeDefaultLocation` 迁移应用且 5 条预置地点 `is_home_default=f` 完好、migrate exit 0、web healthy、HTTPS `/health/live`/`/health/ready`/`/` 均 200、`/locations/search` 正常、磁盘清理完成）；**已知限制**：天地图逆地理编码在生产返回 `{"name":null}`——当前 `tianditu_key` 为浏览器端权限（`301012 Key权限类型为:浏览器端`），而 `/geocoder` 服务端 API 需要服务端权限 Key，按设计 Best-effort 降级留空；待用户浏览器人工验证 5 项需求交互，并视需要申请服务端权限天地图 Key 以启用自动反查 |
