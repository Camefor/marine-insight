...
| 当前任务 ID | `MI-0039` |
| 当前状态 | `DONE` |
| 当前目标 | 后台管理模块（管理员角色种子 + 预置地点 CRUD + 用户列表） |
| 最后完成动作 | 新增 `AdminOptions`（`Admin:Email` 配置）+ 注册端点 `RegisterAsync` 在登录前对 `xuehaq@gmail.com` 自动 `AddToRoleAsync("Administrator")`（失败仅记日志）；SQLite/PostgreSQL 双迁移 `AddAdministratorRole` 固化 `Administrator` 角色（确定性 GUID）并幂等补授目标邮箱；Application 层新增 `AdminLocationService`/`IAdminLocationRepository`/`AdminModels`（名称+经纬度唯一冲突、被预报批次引用禁止删除、级联收藏计数）；Infrastructure 新增 `AdminLocationRepository`（CRUD 落 `audit_logs`：`location.created/updated/deleted`）；Web 新增 `AdminEndpointExtensions`（`GET/POST/PUT/DELETE /api/v1/admin/locations[/{id}]` + `GET /api/v1/admin/users`，RequireAuthorization Administrator + admin 限流 + 写操作防伪头）与 Blazor 后台页 `AdminTabs`/`AdminLocations`/`AdminUsers`；`Operations.razor` 挂 Tab、`AccountNav` 加管理员入口；新错误码 `LOCATION_CONFLICT`/`LOCATION_IN_USE`；`deploy/postgresql-migrations.sql` 已重生成同步；测试新增 24 例（Application 13、Infrastructure 6、Web 5）；已标准提交 `a070024` 并生产部署上线（tar+ssh 同步、pg_dump 备份、`up -d --build` 重建，`AddAdministratorRole` 迁移应用成功、Administrator 角色已固化、web healthy、HTTPS `/health/live`/`/health/ready` 均 200、首页 200、admin 路由匿名 302 跳登录、预置地点 4 条完好） |
| 下一步动作 | git 推送 `origin` 因网络失败待用户经代理/GitHub Desktop 补齐（`github.com:443` 不可达）；待用户浏览器人工验证注册 `xuehaq@gmail.com` 后 `/admin/locations` 增删改、`/admin/users` 列表与非管理员 302 无权访问 |
| 涉及文件 | `Program.cs`、`appsettings.json`、`AccountEndpointExtensions.cs`、`AdminOptions.cs`、`AdminLocationService.cs`、`AdminModels.cs`、`IAdminLocationRepository.cs`、`AdminLocationRepository.cs`、`AdminUserService.cs`、`AdminEndpointExtensions.cs`、`MarineInsightErrorCodes.cs`、`PersistenceServiceCollectionExtensions.cs`、`AdminTabs`/`AdminLocations`/`AdminUsers`、`Operations.razor`、`AccountNav.razor`、双迁移、`deploy/postgresql-migrations.sql`、测试 ×3、`docs/05/06/12/15/16/21/AGENT-GUIDE` |
| 验证结果 | Release 构建 0 警告、0 错误；全量测试 218/218 通过（Domain 51、Application 64、Infrastructure 44、Web 59，新增 24 例）；`dotnet format --verify-no-changes`、`git diff --check` 通过，改动文本文件 BOM/CRLF 已统一；生产部署后冒烟验证全通过 |
| 阻塞/待确认 | 无 |
| 最后更新 | 2026-08-19 |
