...
`MI-0059` 新增 **WorldTides API Key 池后台管理**，将 Key 从「运行期配置文件」升级为「后台可轮换的加密数据库凭证」：

- **加密落库**：新增 `provider_credentials` 表（每 Key 一行），DataProtection 加密存储，`KeyHint` 仅展示末四位；Postgres 生产 / SQLite 本地双轨迁移。现有 compose 挂载的密钥文件（User Secrets / key-per-file）保留为「启动兜底」。
- **多 Key 维护**：可添加多个备选 Key，首个自动激活；后台列表展示每 Key 末四位、健康状态、剩余 credits、告警、最近检查时间和失败原因。
- **激活互斥**：每 Provider 至多一个激活 Key（`IsActive` 部分唯一索引），后台「设为激活」一键切换（先清旧激活再置目标）。
- **自动故障转移**：请求期按「激活优先、其余按创建时间」解析候选队列。认证(401/403)、限流(429)、额度耗尽属 **Key 级失败** → `ReportHealth(failed, reason)` 后自动尝试下一候选；超时/网络/5xx 属非 Key 问题不转移。全部候选失败抛最后的关键失败；DB 与配置均无 Key 时抛 not-configured。
- **健康回写**：成功/失败均回写健康、剩余 credits、告警与检查时间；配置兜底 Key（无 KeyId）不记健康。
- **后台测试连接**：`POST /api/v1/admin/providers/worldtides/credentials/test` 用候选 Key 真实调用一次（固定坐标、最小请求），返回成功/消息/剩余点数，通过后再添加。
- **删除守卫**：激活 Key 且仍存在其他 Key 时拒绝删除，须先激活他者；最后一个激活 Key 允许删除（清空密钥池）。
- **管理入口**：后台 `/admin/providers/worldtides`（Administrator），AdminTabs 新增「潮汐密钥」。运维后续轮换 Key 直接走后台，无需改文件或重启容器。

`MI-0060` 将潮汐改为**登录用户显式按需查询**：基础分析默认不进入 `ITideProvider`，Dashboard 只有登录用户勾选“查询潮汐（消耗 Credits）”后才回源；REST 同样要求 `includeTide=true` 且已认证。缓存命中不产生付费调用日志；每个真实 HTTP 尝试（含故障转移和后台密钥测试）在出网前写入独立费用日志，完成后记录 `callCount`、余额、HTTP、耗时和错误分类。日志写入失败时拒绝发起付费请求，避免不可追踪的额度消耗。
...
| 1.5 | 2026-08-20 | 实现 WorldTides API Key 池：加密落库、多 Key 维护、激活互斥、自动故障转移、健康/credits 回写与后台管理页 |
...
| 1.8 | 2026-08-21 | `MI-0060` 将 WorldTides 改为登录用户显式按需调用，并对每次真实 HTTP 尝试建立可筛选费用日志；缓存命中不记账，日志 Begin 失败则不出网 |
