...
public sealed class MarineAnalysisQueryService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var algorithmVersion = assessments
            .Select(assessment => assessment.AlgorithmVersion)
            .Distinct(StringComparer.Ordinal)
            .Single();
        var cacheIdentity = MarineAnalysisCacheIdentity.Create(
            snapshot.SourceBatches,
            query.Activities,
            algorithmVersion);
    ...
    private async Task<TideQueryStatus> LoadTideAsync(MarineAnalysisQuery query, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            return new TideQueryStatus(
                ...
                result.CreditWarning ? MarineInsight.Application.Errors.MarineInsightErrorCodes.ProviderQuotaExceeded : null,
                result);
        }
